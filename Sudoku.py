    ...
    for i in range(9):
        for j in range(9):
            if defaultgrid[i][j] != 0:
                # Draw a yellow rectangle for non-zero values
                pygame.draw.rect(Window, (255, 255, 0), (i * diff, j * diff, diff + 1, diff + 1))
                # Render the number and draw it on the grid
                value_text = font.render(str(defaultgrid[i][j]), True, (0, 0, 0))
                Window.blit(value_text, (i * diff + 15, j * diff + 15))

    for l in range(10):
        line_thickness = 7 if l % 3 == 0 else 1
        pygame.draw.line(Window, (0, 0, 0), (0, l * diff), (500, l * diff), line_thickness)
        pygame.draw.line(Window, (0, 0, 0), (l * diff, 0), (l * diff, 500), line_thickness)


def fillvalue(value):
    value_text = font.render(str(value), True, (0, 0, 0))
    Window.blit(value_text, (x * diff + 15, z * diff + 15))
...
def gameresult():
    text1 = font.render("Finished", 1, (0, 0, 0))
    Window.blit(text1, (20, 570))
flag=True
flag1 = 0
flag2 = 0
